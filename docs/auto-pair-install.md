...
One-line installer (same as getfreeq.com; re-run anytime to update):
...
State lives under `~/.freeq/`. The installer starts the local node and prints
`freeq pair` next steps.
