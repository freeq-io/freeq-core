...
- Downloads or updates FreeQ Core under `~/freeq-core`
- Builds the daemon and starts the local node
- Stores state under `~/.freeq/` (private keys stay local; no desktop drop folder)
- Opens the local setup page at `http://127.0.0.1:6789/`
...
If you already have a FreeQ checkout and `freeq` on your PATH:
...
Re-run the same one-line installer any time. It updates docs, scripts, and
binaries, then restarts the local node.
