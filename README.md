...
> **Public release repository.** This repo publishes completed FreeQ Core releases
> (`main` = latest release, tags `vX.Y.Z`). Day-to-day development, roadmaps,
> field notes, and internal planning live in a private collaborator repository.
> Please open security reports via [SECURITY.md](SECURITY.md).
...
One-line installer (re-run anytime to update):
...
If you see `PASS`, FreeQ is installed and running locally. Connect peers with
`freeq pair` (see [docs/auto-pair-install.md](docs/auto-pair-install.md)).
...
When direct paths fail, FreeQ uses an accept-only gateway relay: leaves dial
out; the gateway never dials leaves. Design notes:
