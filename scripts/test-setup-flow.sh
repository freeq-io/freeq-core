...
bash -n scripts/setup/freeq-orchestrate-macos.sh
...
assert_file_contains "$TMP_ROOT/simple-install.out" "prebuilt"
assert_file_contains docs/simple-install.md "scripts/install/freeq-install-macos.sh"
...
assert_file_contains docs/simple-install.md "freeq pair host"
assert_file_contains docs/simple-install.md "freeq pair gateway"
assert_file_contains docs/simple-install.md "freeq gateway status"
assert_file_contains docs/simple-install.md "~/.freeq/"
assert_file_contains docs/simple-install.md "prebuilt binaries"
...
assert_file_contains docs/auto-pair-install.md "freeq pair host"
assert_file_contains docs/binary-releases.md "freeq-core-v"
bash -n scripts/install/freeq-platform.sh
bash -n scripts/install/freeq-fetch-release.sh
bash -n scripts/packaging/freeq-package-release.sh
scripts/install/freeq-platform.sh --triple > "$TMP_ROOT/triple.out"
assert_file_contains "$TMP_ROOT/triple.out" "apple-darwin\|unknown-linux"
# Platform helper must always work offline.
scripts/install/freeq-platform.sh --asset-stem v0.2.1 > "$TMP_ROOT/asset.out"
assert_file_contains "$TMP_ROOT/asset.out" "freeq-core-v0.2.1-"
...
assert_file_contains docs/platform-installation-framework.md "freeq gateway status"
assert_file_contains docs/architecture.md "Connectivity Model: Direct First, Self-Healing Relay When Needed"
assert_file_contains docs/self-healing-network-redesign.md "Ten Required Capabilities"
assert_file_contains docs/self-healing-network-redesign.md "FreeQ must not report success until the specific user goal is true"
assert_file_contains docs/self-healing-network-redesign.md "Security is the top objective"
assert_file_contains docs/self-healing-network-redesign.md "The gateway must never assume it can call, dial, or open a fresh inbound"
assert_file_contains docs/self-healing-network-redesign.md "tests prove the gateway never dials a leaf in relay mode"
assert_file_contains docs/gateway-legacy-lessons.md "Status: legacy learning, not the target product architecture"
assert_file_contains docs/gateway-relay-architecture.md "The gateway must not treat"
assert_file_contains docs/gateway-relay-architecture.md "the gateway must never dial back into a leaf node in relay mode"
assert_file_contains docs/podman-relay-lab.md "Do not ask a remote field-test partner"
assert_file_contains scripts/podman/freeq-podman-relay-lab.sh "dataplane_runtime_relays_packet_between_gateway_peers"
assert_file_contains README.md "scripts/podman/freeq-podman-relay-lab.sh --all"
assert_file_contains docs/architecture.md "A gateway is not mandatory"
assert_file_contains docs/architecture.md "does not replace radios"
assert_file_contains README.md "Gateways are optional"
assert_file_contains README.md "direct node-to-node protected overlay"
assert_file_contains README.md "does not secure arbitrary RF waveforms"
assert_file_contains docs/federal-sof-racer-positioning.md "Status: opportunity positioning only"
assert_file_contains docs/federal-sof-racer-positioning.md "direct node-to-node"
assert_file_contains docs/federal-sof-racer-positioning.md "gateway/rendezvous assisted"
assert_file_contains docs/federal-sof-racer-positioning.md "Do not claim"
...
cp "$TMP_ROOT/setup/02-put-peer-file-here/peer-mac-peer.env" "$TMP_ROOT/setup/02-put-peer-file-here/peer-mac-blank-endpoint.env"
perl -0pi -e "s/FREEQ_PUBLIC_ENDPOINT='[^']*'/FREEQ_PUBLIC_ENDPOINT=''/g" "$TMP_ROOT/setup/02-put-peer-file-here/peer-mac-blank-endpoint.env"
if scripts/setup/freeq-validate-peer-env.sh "$TMP_ROOT/setup/02-put-peer-file-here/peer-mac-blank-endpoint.env" > "$TMP_ROOT/validate-blank.out" 2>&1; then
  echo "FAIL: blank endpoint peer env unexpectedly passed strict validation" >&2
  exit 1
fi
assert_file_contains "$TMP_ROOT/validate-blank.out" "FREEQ_PUBLIC_ENDPOINT is missing or blank"
scripts/setup/freeq-validate-peer-env.sh --allow-missing-endpoint "$TMP_ROOT/setup/02-put-peer-file-here/peer-mac-blank-endpoint.env" > "$TMP_ROOT/validate-blank-orchestrator.out"
assert_file_contains "$TMP_ROOT/validate-blank-orchestrator.out" "provided by orchestrator"
rm -f "$TMP_ROOT/setup/02-put-peer-file-here/peer-mac-blank-endpoint.env"
...
assert_file_contains scripts/setup/freeq-orchestrate-macos.sh "no-file-trading setup path"
assert_file_contains scripts/setup/freeq-orchestrate-macos.sh "mode direct"
assert_file_contains scripts/setup/freeq-orchestrate-macos.sh "mode gateway"
assert_file_contains scripts/setup/freeq-orchestrate-macos.sh "FreeQ orchestration result: PASS"
assert_file_contains scripts/setup/freeq-orchestrate-macos.sh "FreeQ orchestration result: FAIL"
assert_file_contains scripts/setup/freeq-orchestrate-macos.sh "rerun with USER@LAN_IP"
assert_file_contains scripts/setup/freeq-orchestrate-macos.sh "ssh-password-only"
scripts/setup/freeq-orchestrate-macos.sh \
  --mode direct \
  --remote user@example.test \
  --dry-run > "$TMP_ROOT/orchestrate-direct-dry-run.out"
assert_file_contains "$TMP_ROOT/orchestrate-direct-dry-run.out" "Mode:             direct"
assert_file_contains "$TMP_ROOT/orchestrate-direct-dry-run.out" "Local endpoint:"
assert_file_contains "$TMP_ROOT/orchestrate-direct-dry-run.out" "Remote endpoint:  example.test:51820"
assert_file_contains "$TMP_ROOT/orchestrate-direct-dry-run.out" "FreeQ orchestration result: PASS"
scripts/setup/freeq-orchestrate-macos.sh \
  --mode gateway \
  --remote user@leaf.example.test \
  --gateway ubuntu@gateway.example.test \
  --gateway-endpoint gateway.example.test:51820 \
  --dry-run > "$TMP_ROOT/orchestrate-gateway-dry-run.out"
assert_file_contains "$TMP_ROOT/orchestrate-gateway-dry-run.out" "Mode:             gateway"
assert_file_contains "$TMP_ROOT/orchestrate-gateway-dry-run.out" "FreeQ orchestration result: PASS"
