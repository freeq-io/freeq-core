...
# Prefer prebuilt binaries (from freeq-fetch-release / GitHub Releases).
# Source build only when forced or binaries are missing.
IDENTITY_BIN=""
if [ -x "${FREEQ_BIN_DIR:-$HOME/.freeq/bin}/freeq-perf-identity" ]; then
  IDENTITY_BIN="${FREEQ_BIN_DIR:-$HOME/.freeq/bin}/freeq-perf-identity"
elif [ -x "$INSTALL_DIR/bin/freeq-perf-identity" ]; then
  IDENTITY_BIN="$INSTALL_DIR/bin/freeq-perf-identity"
elif [ -x "$INSTALL_DIR/target/release/freeq-perf-identity" ]; then
  IDENTITY_BIN="$INSTALL_DIR/target/release/freeq-perf-identity"
fi

if [ -n "$IDENTITY_BIN" ] && [ "${FREEQ_FROM_SOURCE:-0}" != "1" ]; then
  echo "Using prebuilt FreeQ binaries (skip cargo build)."
  echo "  identity tool: $IDENTITY_BIN"
else
  echo "Building FreeQ release binaries from source..."
