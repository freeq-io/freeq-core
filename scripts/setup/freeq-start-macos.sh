...
# Prefer prebuilt freeqd (~/.freeq/bin, package bin/, PATH). Source build only
# when no binary exists and cargo is available (or FREEQ_FROM_SOURCE=1).
resolve_freeqd() {
  local candidates=()
  local c
  if [ -n "${FREEQ_BIN_DIR:-}" ]; then
    candidates+=("${FREEQ_BIN_DIR}/freeqd")
  fi
  candidates+=(
    "${HOME}/.freeq/bin/freeqd"
    "${REPO_ROOT}/bin/freeqd"
    "${REPO_ROOT}/target/release/freeqd"
  )
  if command -v freeqd >/dev/null 2>&1; then
    candidates+=("$(command -v freeqd)")
  fi
...
freeqd_needs_build() {
  if resolve_freeqd >/dev/null 2>&1; then
    if [ "${FREEQ_FROM_SOURCE:-0}" = "1" ] && [ -f "${REPO_ROOT}/Cargo.toml" ]; then
      local bin
      bin="$(resolve_freeqd)"
      case "$bin" in
        */target/release/freeqd)
...
ensure_freeqd_built() {
  local bin
  if bin="$(resolve_freeqd)"; then
    FREEQD_BIN="$bin"
    return 0
  fi
  if ! command -v cargo >/dev/null 2>&1; then
    echo "Missing freeqd binary. Run the installer (downloads prebuilt binaries):" >&2
    echo "  scripts/install/freeq-install-macos.sh" >&2
    exit 1
  fi
  echo "Building freeqd release binary from source..."
  (cd "$REPO_ROOT" && cargo build --release -p freeqd)
  FREEQD_BIN="${REPO_ROOT}/target/release/freeqd"
}
...
if [ -z "${FREEQD_BIN:-}" ]; then
  FREEQD_BIN="$(resolve_freeqd || true)"
fi
if [ -z "${FREEQD_BIN:-}" ] || [ ! -x "$FREEQD_BIN" ]; then
  echo "freeqd binary not found. Run: scripts/install/freeq-install-macos.sh" >&2
  exit 1
fi
echo "freeqd binary: $FREEQD_BIN"
nohup sudo env ${sudo_env[@]+"${sudo_env[@]}"} "$FREEQD_BIN" --config "$CONFIG" --foreground > "$LOG_FILE" 2>&1 &
pid="$!"
echo "$pid" > "$PID_FILE"
echo "freeqd pid: $pid"
echo "log: $LOG_FILE"
