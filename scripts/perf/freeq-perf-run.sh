...
Examples:
  ...
  scripts/perf/freeq-perf-run.sh --target 203.0.113.10 --overlay-host 10.66.0.1 --mode both
...
for i in range(5):
    ...
    rc = subprocess.call(["ssh", "-o", "BatchMode=no", "-o", "ConnectTimeout=8", target, "true"])
...
start = time.perf_counter()
rc = subprocess.call(["scp", "-q", payload, remote])
elapsed = time.perf_counter() - start
mb = int(sys.argv[3])
print(f"rc={rc}")
print(f"seconds={elapsed:.3f}")
print(f"mbps={(mb * 8) / elapsed:.2f}")
subprocess.call(["ssh", target, "rm -f /tmp/freeq-perf-payload.bin"])
' "$payload" "$ssh_target" "$SCP_MB"
}
...
run_leg() {
  ...
  time_ssh "${prefix}-ssh-latency" "$host"
  scp_test "${prefix}-scp-upload" "$host"
...
if [ "$MODE" = "direct" ] || [ "$MODE" = "both" ]; then
  run_leg "direct" "$TARGET_HOST"
fi
if [ "$MODE" = "freeq" ] || [ "$MODE" = "both" ]; then
  run_leg "freeq" "$OVERLAY_HOST"
fi
