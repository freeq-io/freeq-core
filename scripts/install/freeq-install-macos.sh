...
What this does:
  ...
  2. Builds FreeQ and creates this Mac's local node identity under ~/.freeq/.
  3. Starts FreeQ in local listen mode and checks the local API.
  4. Does NOT require ~/FreeQ drop folders for pairing (legacy paths optional).

After install, connect without human peer-file trading:
  freeq pair host --code SECRET --port 8791 --auto-start     # direct, reachable side
...
State folder:
  ~/.freeq/   (identity, peers/received, pair/, perf/)
...
say "  scripts/setup/freeq-orchestrate-macos.sh --mode direct --remote user@host"
say ""
say "Rollback:"
say "  freeq stop"
say ""
