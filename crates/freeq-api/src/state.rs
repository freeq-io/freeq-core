...
struct RuntimeState {
    ...
    local_peer_env_path: Option<PathBuf>,
}
    ...
    pub fn new(
        ...
        Self {
            inner: Arc::new(RwLock::new(RuntimeState {
                ...
                known_peers: Vec::new(),
                local_peer_env_path: None,
            })),
        }
    }

    /// Record the local public peer exchange file generated by setup.
    pub async fn set_local_peer_env_path(&self, path: PathBuf) {
        self.inner.write().await.local_peer_env_path = Some(path);
    }

    /// Return the local public peer exchange file generated by setup.
    pub async fn local_peer_env_path(&self) -> Option<PathBuf> {
        self.inner.read().await.local_peer_env_path.clone()
    }

    /// Return the folder where received public peer files should be installed.
    ///
    /// Canonical auto-pair path is `~/.freeq/peers/received` (derived from the
    /// local public peer.env location when it lives under `.freeq/perf/`).
    /// New tooling must not require legacy `~/FreeQ/02-put-peer-file-here`.
    pub async fn peer_receive_dir(&self) -> Option<PathBuf> {
        let local = self.local_peer_env_path().await?;
        // .../.freeq/perf/peer.env -> .../.freeq/peers/received
        if let Some(perf_dir) = local.parent() {
            if let Some(freeq_home) = perf_dir.parent() {
                if freeq_home
                    .file_name()
                    .and_then(|s| s.to_str())
                    .is_some_and(|n| n == ".freeq")
                {
                    return Some(freeq_home.join("peers").join("received"));
                }
            }
        }
        let home = std::env::var_os("HOME").map(PathBuf::from)?;
        Some(home.join(".freeq").join("peers").join("received"))
    }
