...
impl ApiState {
    ...
    /// Return the visible folder where received public peer files should be installed.
    pub async fn peer_receive_dir(&self) -> Option<PathBuf> {
        let path = self.local_peer_env_path().await?;
        let home = path.parent()?.parent()?.parent()?;
        Some(home.join("FreeQ").join("02-put-peer-file-here"))
    }
