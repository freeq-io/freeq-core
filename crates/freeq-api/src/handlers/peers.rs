...
pub async fn join_invite(
    ...
    Ok(Json(InviteJoinResponse {
        ...
        activation_command: Some(
            "freeq pair connect   # or: scripts/setup/freeq-connect-macos.sh --restart".into(),
        ),
        message: "PASS: invite accepted; peer installed under ~/.freeq/peers/received.".into(),
    }))
}
...
mod tests {
    ...
    async fn join_invite_records_known_peer_and_installs_peer_file() {
        ...
        assert!(joined
            ...
            .as_deref()
            .unwrap_or_default()
            .contains("freeq pair connect"));
        assert_eq!(api_state.known_peers().await.len(), 1);
        // Canonical install path: ~/.freeq/peers/received (relative to test root)
        ...
        let installed = fs::read_to_string(&installed_path)
            .unwrap_or_else(|_| panic!("read installed peer file at {}", installed_path.display()));
